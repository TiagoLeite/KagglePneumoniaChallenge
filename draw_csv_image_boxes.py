...
# Best result 0.174
df = pd.read_csv('scored_194.csv')
df2 = pd.read_csv('scored_180.csv')
...
for count in range(number_images):
    ...
    if not isNaN(df['PredictionString'][count]):
        ...
        while df['PredictionString'][count].split(" ")[space] != "":
            ...
            x = float(df2['PredictionString'][count].split(" ")[space + 1]) * resize_factor
            y = float(df2['PredictionString'][count].split(" ")[space + 2]) * resize_factor
            width = float(df2['PredictionString'][count].split(" ")[space + 3]) * resize_factor
            height = float(df2['PredictionString'][count].split(" ")[space + 4]) * resize_factor
            box_area[num_boxes] = int((width * height) / (resize_factor * 1000 * resize_factor))
            pts_box = np.array([[x, y], [x, y + height], [x + width, y + height], [x + width, y]], np.int32)
            cv.polylines(res, [pts_box], True, (255, 0, 0), 1)
